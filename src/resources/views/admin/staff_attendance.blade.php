...
<main class="attendance-list">
    ...
    <div class="month-nav">
        <a href="/admin/staff/{{ $user->id }}?month={{ $previousMonth }}">
            ← 前月
        </a>

        <p>{{ $currentMonth->format('Y/m') }}</p>

        <a href="/admin/staff/{{ $user->id }}?month={{ $nextMonth }}">
            翌月 →
        </a>
    </div>
    ...
    <table class="attendance-table">
        ...
        @foreach ($attendances as $attendance)
            <tr>
                <td>{{ \Carbon\Carbon::parse($attendance->work_date)->isoFormat('M/D(ddd)') }}</td>
                <td>
                    {{ $attendance->clock_in
